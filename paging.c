...
void add_to_index(char *, char *, indextable **);
	...
	while(i < words) {
		...
		while(j < s) {
			if(strcmp(keywords[i], ds[j]->keyword) == 0) {
				add_to_index(keywords[i], location, &ds[j]);
				j = s + 1;
				break;
			}
			j++;
		}
		if(j == s + 1) add_to_index(keywords[i], location, NULL);
		i++;

	}
}

void add_to_index(char * word, char * loc, indextable **ds) {
